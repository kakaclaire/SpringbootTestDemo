...
public class GlobalExceptionHandler {
    ...
    public ResultDto serviceExceptionHandler(ServiceExpection se) {
        //打印日志
...
//        return "系统错误  系统繁忙，请稍后重试";
        return ResultDto.failure("系统错误  系统繁忙，请稍后重试");
    }
    ...
    public ResultDto resultFormat(Throwable t) {
        ...
        if (t instanceof ServiceExpection) {
            return ResultDto.failure("业务异常" + tips);
        }
        if(t instanceof Exception){
            return ResultDto.failure("非业务异常" + tips);
        }return ResultDto.failure(tips);
    }
